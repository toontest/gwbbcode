...
var GWConfig = {
  ...
  js: {
    data: {
      en: "https://toontest.github.io/gwbbcode/public/gwbbcode/data/en.js",
    },
  ...
  templates: {
    skill:
      '<span class="skill skillPack[pack] skill[id] [cls]"><span class="skill-[type]"></span><span class="skill-[elite]"></span></span>',
    detail:
    ...
    build:
      '<div class="gw-build gw-count[count]"><div class="gw-panel"><div class="gw-prof"><span class="gw-prof[prof1]" title="[profname1]"></span><span class="gw-prof[prof2]" title="[profname2]"></span></div></div><span class="gw-container"><span class="gw-skills"><span class="gw-skills-center">[skills]</span></span><span class="gw-attr">[attrs]</span><span class="gw-template">[template]</span></span></div>',
  },
  detail: {
    offset: {
      x: 5,
...
function GWBuild() {
  ...
  this.findAttributeByInt = function (i) {
	var long_attrs = dict_reverse(GWConfig.db.long_attributes);
	if(typeof long_attrs[i] != "function" && typeof long_attrs[i] != "undefined") {
		return long_attrs[i];
	}
    return null;
  };
  ...
  this.getAttributes = function () {
    var attributes = {},
      n = _self._attributes.length;
    for (var i = 0; i < n; i++) {
      attributes[_self.findAttributeByInt(_self._attributes[i])] =
        _self._attributeValues[i];
    }
    return attributes;
  };
  this.getSkills = function () {
    var skills = [],
      n = _self._skills.length;
    for (var i = 0; i < n; i++) {
  ...
  this.getHtml = function () {
    ...
    for (var i in attributes) {
      if (typeof attributes[i] != "function" && i != "null") {
		if(counter > 0) {
			attrs += ", ";
		}
    ...
    tpl = tpl.replace(
      "[count]",
      GWConfig.forceFullBar ? 8 : _self.skillsCount()
    );
    tpl = tpl.replace("[attrs]", attrs || "");
    tpl = tpl.replace(
      "[prof1]",
      GWConfig.db.professionkeys[_self.getProfession1()] || ""
    );
    tpl = tpl.replace(
      "[profname1]",
      GWConfig.db.professionnames[_self.getProfession1()] || ""
    );
    tpl = tpl.replace(
      "[prof2]",
      GWConfig.db.professionkeys[_self.getProfession2()] || ""
    );
    tpl = tpl.replace(
      "[profname2]",
      GWConfig.db.professionnames[_self.getProfession2()] || ""
    );
...
function GWSkill() {
  ...
  this.getHtmlDetail = function () {
    ...
    tpl = tpl.replace(
      "[attr]",
      (long_attr || "") +
        (_self.attrlevel ? " <b>" + _self.attrlevel + "</b>" : "")
    );
...
function GWBindEvents(jQuerySelector) {
  ...
  jQuerySelector.find(".skill, .skill-text").each(function () {
    var cls = jQuery(this).get(0).className,
      m;
  ...
  jQuerySelector.each(function () {
    ...
    while ((m = html.match(/\[\[(.*?)\]/i))) {
      ...
      if (GWConfig.db.search[skillkey]) {
        ...
        html = html.replace(
          match,
          '<a href="javascript:void(0)" class="skill-text skill-text' +
            skillid +
            " " +
            cls +
            '">' +
            GWConfig.db.skills[skillid].n +
            "</a>"
        );
      } else {
...
function GWBindSkill(domobject, idskill) {
  ...
  skill.setId(idskill);
  if ($build.length == 1) {
    var cls = $build.get(0).className,
      m;
    if (
      (m = cls.match(new RegExp("gw-attr-" + skill.attr + "-([0-9]+)", "i")))
    ) {
      skill.attrlevel = m[1];
  ...
  if (
    (m = domobject.className.match(
      new RegExp("gw-attr-" + skill.attr + "-([0-9]+)", "i")
    ))
  ) {
    skill.attrlevel = m[1];
    divId += skill.attr + m[1];
